...
func (controller filesController) StoreUserPicBase64(c *gin.Context) {
	...
	var fileRequestBody FileRequestBody
	if err := c.ShouldBindBodyWith(&fileRequestBody, binding.JSON); err != nil {
		FilesController.BadRequestResponse(c)
		return
	}

	file, err := base64.StdEncoding.DecodeString(fileRequestBody.ToString())
	if err != nil {
		FilesController.BadRequestResponse(c)
		return
	}
